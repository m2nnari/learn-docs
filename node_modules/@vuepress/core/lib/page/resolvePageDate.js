...
const resolvePageDate = ({ frontmatter, filePathRelative, }) => {
    ...
    if (shared_1.isString(frontmatter.date)) {
        return shared_1.formatDateString(frontmatter.date, DEFAULT_DATE);
    }
    ...
    if (filename) {
        ...
        if (matches) {
            return shared_1.formatDateString(`${matches[1]}-${matches[2]}-${(_a = matches[3]) !== null && _a !== void 0 ? _a : '01'}`, DEFAULT_DATE);
        }
    }
    ...
    if (dirname) {
        ...
        if (matches) {
            return shared_1.formatDateString(`${matches[1]}-${matches[2]}-${(_b = matches[3]) !== null && _b !== void 0 ? _b : '01'}`, DEFAULT_DATE);
        }
    }
    return DEFAULT_DATE;
};
