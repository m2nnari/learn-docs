...
const createBuild = (options) => async (app) => {
    ...
    await utils_1.withSpinner('Compiling with webpack')(async () => {
        ...
        const clientConfig = utils_2.resolveWebpackConfig({
            config: createClientConfig_1.createClientConfig(app, options),
            options,
            isServer: false,
            isBuild: true,
        });
        const serverConfig = utils_2.resolveWebpackConfig({
            config: createServerConfig_1.createServerConfig(app, options),
            options,
            isServer: true,
            isBuild: true,
        });
    ...
    await utils_1.withSpinner('Rendering pages')(async () => {
        ...
        const { createServerApp } = require(app.dir.dest('.server/app'));
        // create vue ssr app
    ...
    // print success log
    utils_1.logger.success('VuePress webpack build successfully!');
};
